...
@section('content')
  <div class="admin-section"
    ...
    <div class="admin-pp-wrap mt-4">
      ...
      <template v-else>
        <div class="card-body">
          <x-pesan-tengah title="Data Pegawai Kosong" />
        </div>
      </template>
    </div>
  </div>
      ...
      window.pegawaiApp = new Vue({
        ...
        methods: {
          ...
          loadData() {
            ...
            axios.post(`{{ route('admin.pegawai.datatable') }}`)
              .then(res => {
                if (res?.data?.data) {
          ...
          ubahOnClick(item) {
            ...
            axios.post(`{{ route('admin.pegawai.edit') }}`, {
                id: item.id,
              })
              .then(res => {
                if (res?.data?.success) {
          ...
          simpanClick() {
            ...
            switch (this.mode) {
              case 'tambah':
                ...
                axios.post(`{{ route('admin.pegawai.store') }}`, dataKirim)
                  .then(res => {
                    if (res.data?.success) {
                      this.list.push({
                        id: res.data.data.id,
              ...
              case 'ubah':
                ...
                axios.post(`{{ route('admin.pegawai.update') }}`, dataKirim)
                  .then(res => {
                    if (res.data?.success) {
          ...
          hapusOnClick(item, index) {
            AKHelper.DeleteConfirm({
              ...
              url: `{{ route('admin.pegawai.destroy') }}`,
              data: {
                id: item.id,
              },
              done: (res) => {
          ...
          saveOrder() {
            ...
            axios.post(`{{ route('admin.pegawai.order') }}`, {
                _method: 'PUT',
                order: urutan,
              })
              .then(res => {
